function [fixedPointConditionsBest, JacobianBest, eigenValueMaxMean, resemblanceMetricsFunctionBest, resemblanceMetricsSolutionBest, isStateConverged, differenceMaxSolutionVector] = calculateFixedPointConditionsWithJacobianThesis(initialConditions, simulationParameters, varargin)
...
p.addParameter('tol', 1e-20);                                                % The tolerance of that indicates that the newton raphson method is converged
p.addParameter('max_iter', 30);                                             % Maximum N.R. iterations
...
p.addParameter('maximumNoProgressSteps', 7);                                % The maximum number of step that the N.R. convergence need to have progressed.
p.addParameter('progressRateMinimum', 0);                                 % The minimum progress that must happen after maximumNoProgressSteps.
...
while ~isStateConverged
    ...
    differenceMaxFunction                                       = resemblanceMetricsFunction.DifferenceAbsolute.Maximum;
    ...
    differenceMaxSolution                                       = resemblanceMetricsSolution.DifferenceAbsolute.Maximum;
    jSolution                                                   = jSolution + 1;
    differenceMaxSolutionVector(jSolution)                      = differenceMaxSolution;
    ...
    if ~isStateConverged
        if resemblanceMetricsSolution.DifferenceAbsolute.Maximum > maximumStateDifference * (1 - progressRateMinimum)
            noProgressIterations = noProgressIterations + 1;
        else
            noProgressIterations = 0;
            maximumStateDifference = resemblanceMetricsSolution.DifferenceAbsolute.Maximum;
        end
...
DifferenceAbsolute  = resemblanceMetricsFunction.DifferenceAbsolute;
fieldNames          = fieldnames(DifferenceAbsolute);
...
for iField = 1 : numOfFields
    ...
    fieldValue  = DifferenceAbsolute.(fieldName);
