function results = run_SSP1TillMaxHeight_GC(simulationParameters,options,results)
%% run_SSP1TillMaxHeight_GC 
...
if results.fell==0
    ...
    try
        ...
        [tMaxHeightLeg1,xMaxHeightLeg1] = ode15s(@(ti,xi)  SSP_beta14x1(ti,xi,parametersModel)+xElegxos4x1(xi,1,parametersModel), parametersSolver.tspan, results.intermediate.xinit, options.optMaxHeight1);
 	...
 	% Merge the results
    results = export_results(tMaxHeightLeg1,xMaxHeightLeg1,parametersModel,results,1);
