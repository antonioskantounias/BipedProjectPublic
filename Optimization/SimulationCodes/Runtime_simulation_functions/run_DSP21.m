function results = run_DSP21(simulationParameters,options,results)
%% run_DSP21 
% Description:  This function runs the simulation in the double stance phase till the foot1 take off occures.
...
% Check if the foot's 2 reaction force is possitive before the inpact occurance
if Ls4x1a(4)>0
    
    % Check if after the impact occurance the foot would remain in contact with the ground
    [~, isImpactValid,~] = calculateImpactSSP2_DSP21(results.intermediate.xinit, parametersModel);
    
    if isImpactValid
        % Run the simulation
        try
            % Try running the simulation with high tolerance
            [t21,x21] = ode23t(@(ti,xi)  DSP_beta16x1(ti,xi,parametersModel), parametersSolver.tspan, results.intermediate.xinit, options.opt_DSP2);
        ...
        % Merge the results
        results = export_results(t21,x21,parametersModel,results,21);
