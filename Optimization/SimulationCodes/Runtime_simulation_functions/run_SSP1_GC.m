function results = run_SSP1_GC(simulationParameters, options, results)
%% run_SSP1_GC 
% Description:  This function runs the simulation in single stance phase of the foot 1 till the foot2 heel strike occures.
%               Also the function merges the simulation results with previous results. A gravity compensation control is implemented.
% 
...
if results.fell==0
    ...
    try 
        ...
        [t1,x1] = ode15s(@(ti,xi)  SSP_beta14x1(ti,xi,parametersModel)+xElegxos4x1(xi,1,parametersModel), parametersSolver.tspan, results.intermediate.xinit, options.opt_SSP1);
