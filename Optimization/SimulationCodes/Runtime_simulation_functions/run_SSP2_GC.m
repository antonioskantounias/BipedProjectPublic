function results = run_SSP2_GC(simulationParameters, options, results)
%% run_SSP2 
% Description:  This function runs the simulation in single stance phase of the foot2 till the foot1 heel strike occures.
%               Also the function merges the simulation results with previous results. Gravity compensation control is implemented.
% 
...
if results.fell==0
    ...
    try 
        ...
        [t2,x2] = ode15s(@(ti,xi)  SSP_beta14x1(ti,xi,parametersModel)+xElegxos4x1(xi,2,parametersModel), parametersSolver.tspan, results.intermediate.xinit, options.opt_SSP2);
    ...
    % Merge the results
    results = export_results(t2,x2,parametersModel,results,2);
...
numOfEventOccurance                                     = results.events.occurancies.HS1 + 1;
results.events.occurancies.HS1                          = numOfEventOccurance;
results.events.intermediates(numOfEventOccurance).HS1   = results.intermediate;
results.events.intermediates(numOfEventOccurance).HS1   = exportStatesFromXVector(results.events.intermediates(numOfEventOccurance).HS1, results.intermediate.xinit, parametersModel);
